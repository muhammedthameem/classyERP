...
content = content.replace(
  /updatedClient\.measurements\[selectedMeasurementIndex\] = \{([\s\S]*?)note: measurementToSave\.note\n        \}/,
  (match) => match.replace('note: measurementToSave.note', 'note: measurementToSave.note,\n          photoUrl: measurementToSave.photoUrl || null')
);

const viewModeNoteStr = `{currentMeasurement.note && (`;
const viewModePhotoStr = `
...
if (!content.includes('Measurement Photo')) {
    content = content.replace(viewModeNoteStr, viewModePhotoStr + '\n            ' + viewModeNoteStr);
}

const editModeNoteStr = `{/* Note Section */}`;
const editModePhotoStr = `
...
content = content.replace(editModeNoteStr, editModePhotoStr + '\n            ' + editModeNoteStr);
