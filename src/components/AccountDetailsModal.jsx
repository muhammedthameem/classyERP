...
function AccountDetailsModal({ fullUser, onClose, onChanged, onLogout, themeStyle }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] grid place-items-center bg-black/60 px-4 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-[32px] border border-[var(--border)] bg-[var(--surface-strong)] p-8 shadow-[var(--shadow)] relative max-h-[90vh] overflow-y-auto text-[var(--text)]">
        
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-full hover:bg-[var(--soft)] transition text-[var(--muted)]"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>

        <div className="flex items-center gap-4 mb-8">
          <div className="h-16 w-16 rounded-2xl bg-[var(--accent)] grid place-items-center shadow-lg shadow-[var(--accent)]/20">
            <User size={32} className="text-white" />
          </div>
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-[var(--text)]">Security Settings</h2>
            <p className="text-[var(--muted)] text-sm">Update your profile and access keys</p>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 mb-8">
          <div className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)]">
            <div className="flex items-center gap-2 text-[var(--muted)] text-[10px] font-bold uppercase tracking-widest mb-2">
              <Mail size={14} /> Email
            </div>
          ...
          <div className="p-4 rounded-2xl bg-[var(--surface)] border border-[var(--border)]">
            ...
            <p className="font-medium text-[var(--accent)]">{fullUser?.designation || 'Staff'}</p>
          </div>
        </div>
